...
class SquareAttack():
    ...
    def margin_and_loss(self, x, y):
        ...
        mod = self.mod.copy()
        mod.update({'indiv': 1, 'labels': y})
        if self.loss == 'margin':
            self.loss = 'hinge'
        elif self.loss == "logit":
            # self.loss = 'hinge'
            match_target = extract_target_logits(self.predict, x, y)
            mod.update({"match_target": match_target})
        loss_fn = get_loss_fn(model=self.predict, labels=y, type=self.loss, targeted=self.targeted, modifier=mod)
        _, indiv_loss, y_result = loss_fn(x)
        y_corr, y_others = y_result

        if not self.targeted:
            return y_corr - y_others, -indiv_loss
        else:
            return y_others - y_corr, -indiv_loss
    ...
    def perturb(self, x, y=None):
        ...
        """
        if isinstance(y, TargetedMisclassification):
            y = y.target_classes.raw
            self.targeted = True
        else:
            self.targeted = False
